...
Test("accepts reconnection state after neutralization", () => { var b = new FakeGamepadBackend(); b.CreateSlots(); using var e = new BridgeEngine(b); Protocol.TryParseState(State(sequence:1,buttons:"{\"a\":true}"), out var first, out _); Protocol.TryParseState(State(sequence:2,buttons:"{\"b\":true}"), out var second, out _); e.Apply(first!, DateTimeOffset.UtcNow); e.Neutralize(1); Assert(e.Apply(second!, DateTimeOffset.UtcNow) && b.States[1].Buttons["b"], "reconnect"); });
