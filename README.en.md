...
> Experimental `1.4.0-beta.3` adds local QR pairing, a Windows installer, a portable package, and an optional dual virtual-controller/XInput mode. The v1.3 keyboard mode remains available for emulators.
...
- Temporary in-memory PIN, local QR pairing, and local-network-only communication.
...
4. Scan the automatic QR shown in the terminal, confirm the player, and tap **Connect**.
...
The `1.4.0-beta.3` prerelease includes a single Windows installer with an embedded Python runtime and a self-contained .NET bridge. QR symbols are generated entirely offline by a vendored MIT library; the PIN is removed from the phone URL after loading and is never stored in localStorage. It creates separate **Emulators — Keyboard** and **PC Games — Virtual controller** shortcuts. Download only from the official `cristianohsm/celular-gamepad-pc` GitHub releases and verify the published SHA-256 checksum.
